...
class App {
  ...
  updateMonitorUI(status) {
    ...
    if (statCpu)
      statCpu.textContent = sysCpuVal.toFixed(2) + "%";
    if (statProcCpu)
      statProcCpu.textContent = procCpuVal.toFixed(2) + "%";
    ...
    if (cpuProgress)
      cpuProgress.style.width = Math.max(sysCpuVal, procCpuVal) + "%";
    if (sysCpuText)
      sysCpuText.textContent = sysCpuVal.toFixed(2) + "%";
    if (procCpuText)
      procCpuText.textContent = procCpuVal.toFixed(2) + "%";
    ...
    if (statMemPerc)
      statMemPerc.textContent = sysMemVal.toFixed(2) + "%";
    if (statProcMemPerc)
      statProcMemPerc.textContent = procMemVal.toFixed(2) + "%";
    if (statMemAbs)
      statMemAbs.textContent = this.formatFileSize(status.memory);
    ...
    if (memProgress)
      memProgress.style.width = sysMemVal + "%";
    if (sysMemText)
      sysMemText.textContent = sysMemVal.toFixed(2) + "%";
    if (procMemText)
      procMemText.textContent = procMemVal.toFixed(2) + "%";
    ...
    if (statUsers)
      statUsers.textContent = status.users;
    if (statDevices)
      statDevices.textContent = status.devices;
    if (statUptime)
      statUptime.textContent = this.formatUptime(status.uptime);
...
此操作将删除歌单及其中的所有歌曲！`, { danger: true }))
  ...
  async deleteSong(playlistIndexOrType, songIndex) {
    ...
    if (!song)
      return;
    if (!await showSelect("删除歌曲", `确定要从 "${playlist.name}" 中删除歌曲 "${song.name}" 吗？`, { danger: true }))
      return;
    try {
  ...
  async batchDeleteSongs() {
    ...
    if (!playlist)
      return;
    if (!await showSelect("批量删除", `确定要删除选中的 ${checkboxes.length} 首歌曲吗？`, { danger: true }))
      return;
    try {
  ...
  async loadConfig() {
    try {
      ...
      const navPlayerLink = document.getElementById("nav-player-link");
      if (navPlayerLink)
        navPlayerLink.href = config["player.path"] === "" ? "/" : config["player.path"] ?? "/music";
  ...
  async saveConfig(silent = false) {
    ...
    try {
      ...
      const navPlayerLink = document.getElementById("nav-player-link");
      if (navPlayerLink)
        navPlayerLink.href = playerPath === "" ? "/" : playerPath ?? "/music";
      if (!silent) {
        if (res.warning) {
          showInfo(`配置保存成功！
  ...
  async loadLogs() {
    ...
    try {
      ...
      if (data.logs && data.logs.length) {
        container.innerHTML = data.logs.filter((line) => line.trim()).map((line) => `<div class="log-line">${this.escapeHtml(line)}</div>`).join("");
...
确定要继续吗？`, { danger: true }))
  ...
  async syncFilesToWebDAV() {
    if (!await showSelect("同步文件", "确定要强制同步所有文件到 WebDAV 吗？"))
      return;
    const statusEl = document.getElementById("sync-status-content");
  ...
  renderSongNameCell(song) {
    ...
    const coverHtml = picUrl ? `<img src="${picUrl}" class="song-cover" loading="lazy" alt="cover" onerror="this.style.opacity=0">` : `<div class="song-cover" style="background: rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center;">\uD83C\uDFB5</div>`;
    const singerHtml = song.singer ? `<span class="song-singer-mobile">${this.escapeHtml(song.singer)}</span>` : "";
    return `
...
${isDirectory ? "⚠️ 文件夹内的所有内容也会被删除！" : ""}`, { danger: true }))
      return;
    try {
      await this.request("/api/files", {
        method: "DELETE",
  ...
  async deleteSnapshot(id) {
    if (!await showSelect("删除快照", "确定要删除这个快照吗？", { danger: true }))
      return;
    const username = document.getElementById("snapshot-user-select")?.value;
    if (!username)
      return;
