...
export default {
    async getList(uid: any, tryNum: any= 0): Promise<any> {
        ...
        const url = `${APIURL}?${Object.keys(params).map((k: any) => `${k}=${encodeURIComponent(params[k])}`).join('&')}`
