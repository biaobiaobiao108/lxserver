...
export default {
    ...
    getArtistDetail(id: any) {
        return weapiRequest('/artist/head/info/get', { id }).promise.then(({ body }: any) => {
    ...
    getArtistSongs(id: any, page: any= 1, limit: any= 100, order: any= 'hot') {
        return weapiRequest('/v1/artist/songs', {
            ...
            return {
                list: musicDetail.filterList({ songs: body.songs, privileges: body.songs.map((s: any) => s.privilege || {}) }),
                total: body.total,
                source: 'wy',
            }
        })
     ...
     * @param {*} id 歌手 ID
     */
    getArtistAlbums(id: any, page: any= 1, limit: any= 50) {
        return weapiRequest(`/artist/albums/${id}`, {
            ...
            return {
                source: 'wy',
                list: (body.hotAlbums || []).map((item: any) => ({
                    id: item.id,
                    name: item.name,
                    img: item.picUrl,
    ...
    getAlbumSongs(id: any) {
        return weapiRequest(`/v1/album/${id}`, {}).promise.then(({ body }: any) => {
            ...
            return {
                list: musicDetail.filterList({
                    songs,
                    privileges: songs.map((s: any) => s.privilege || { id: s.id })
                }),
                total: songs.length,
