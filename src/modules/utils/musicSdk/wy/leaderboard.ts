...
export default {
  ...
  async getList(bangid: any, page: any, retryNum: any= 0): Promise<any> {
    ...
    let musicDetail
    try {
      musicDetail = await musicDetailApi.getList(resp.body.playlist.trackIds.map((trackId: any) => trackId.id))
    } catch (err: any) {
      console.log(err)
      if (err.message == 'try max num') {
        throw err
