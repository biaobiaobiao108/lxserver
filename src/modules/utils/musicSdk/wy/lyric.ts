...
// const parseLyric = (str: any, lrc: any) => {
//   if (!str) return ''

//   str = str.replace(/\r/g, '')

//   let lxlyric = str.replace(/\[((\d+),\d+)\].*/g, (str: any) => {
...
//     let startTime = 0
//     str = str.replace(/\(0,1\) /g, ' ').replace(/\(\d+,\d+\)/g, (time: any) => {
...
const parseTools = {
  ...
  msFormat(timeMs: any) {
    ...
    let ms = timeMs % 1000
    timeMs /= 1000
    let m = Math.trunc(timeMs / 60).toString().padStart(2, '0')
    timeMs %= 60
